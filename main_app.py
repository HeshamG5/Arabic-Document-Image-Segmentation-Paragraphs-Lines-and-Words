...
CROPPED_LINES_DIR = Path("cropped_lines")   # مجلد جديد لحفظ الأسطر المقطوعة
...
st.title("🖼️ Document Line & Paragraph Segmentation")
...
@st.cache_resource
def load_line_model():
    """Load YOLOv5 model using torch.hub and force GPU."""
    ...
    try:
        ...
        # نقل النموذج إلى الجهاز المطلوب
        model.to(device)
        # التأكد من نقل الأجزاء الداخلية
...
def extract_and_save_lines(paragraph_img_bgr, boxes, output_dir, prefix):
    """
    Given a paragraph image (BGR), a list of bounding boxes (x1,y1,x2,y2) and an output directory,
    crop each line and save as PNG. Returns list of (index, cropped_line_rgb).
    """
    cropped_lines = []
    for i, (x1, y1, x2, y2, score) in enumerate(boxes):
        # ensure coordinates are integers
        x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
        # Crop line from paragraph image
...
# -------------------- Line Detection functions (original logic, but model on GPU) --------------------
...
def detect_lines_on_image(image_path, model, device):
    ...
    # Return boxes as list of (x1,y1,x2,y2,score)
...
def process_all_paragraphs(crop_dir, model, device, output_dir, clear_first=True):
    """
    Process each paragraph image: detect lines, draw boxes, save detected image,
    and also crop individual lines and save them.
    Returns:
        processed: list of (stem, drawn_image)
        all_cropped_lines: list of (line_image, filename) for all lines from all paragraphs
    """
    if clear_first and output_dir.exists():
        shutil.rmtree(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Prepare folder for cropped lines
    ...
    processed = []
    all_cropped_lines = []   # store (line_img, filename)
    ...
    for idx, img_path in enumerate(image_files):
        ...
        if drawn is not None:
            # Save detected image (with boxes)
            ...
            # Now crop and save individual lines
            # Read original image for cropping (BGR)
            orig_img = cv2.imread(str(img_path))
            if orig_img is not None and boxes:
                # Use the boxes (which are in the original image coordinates) to crop lines
                # boxes are list of (x1,y1,x2,y2,score)
                for line_idx, (x1, y1, x2, y2, score) in enumerate(boxes):
                    ...
                    line_rgb = cv2.cvtColor(line_crop, cv2.COLOR_BGR2RGB)
                    # Create a unique filename: original_paragraph_stem_line_index.png
...
def main():
    ...
    if "cropped_lines" not in st.session_state:
        st.session_state.cropped_lines = None   # will store list of (line_img, filename)
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.segmentation_done and line_model is not None:
                ...
                with st.spinner("Detecting lines in all paragraph images and cropping lines..."):
                    try:
                        ...
                        if processed:
                            ...
                            # Prepare ZIP buffer for detected images (with boxes)
                            ...
                            # Also create a separate ZIP for cropped lines
                            # We'll create a separate download button later
                        else:
            ...
            if st.session_state.line_detection_done and st.session_state.line_zip_buffer is not None:
                ...
                # Download button for detected images (with boxes)
                st.download_button(
                ...
                # Show and download cropped lines
                if st.session_state.cropped_lines and len(st.session_state.cropped_lines) > 0:
                    st.subheader("✂️ Cropped Lines (individual lines)")
                    # Display lines in a scrollable container
                    line_display_cols = st.columns(4)  # 4 columns for grid
                    lines_per_col = len(st.session_state.cropped_lines) // 4 + 1
                    for idx, (line_img, filename) in enumerate(st.session_state.cropped_lines):
                        ...
                        with line_display_cols[col]:
                            ...
                            # optionally add a download button per line
                            # but we'll also provide a ZIP
                    # Create ZIP of all cropped lines
                    lines_zip_buffer = io.BytesIO()
                    with zipfile.ZipFile(lines_zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                        for line_img, filename in st.session_state.cropped_lines:
                            img_pil = Image.fromarray(line_img)
                            img_bytes = io.BytesIO()
                            img_pil.save(img_bytes, format="PNG")
                            zip_file.writestr(filename, img_bytes.getvalue())
                    lines_zip_buffer.seek(0)
                    st.download_button(
                        label="📦 Download all cropped lines as ZIP",
                        data=lines_zip_buffer,
                        file_name=f"cropped_lines_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip",
                        mime="application/zip"
                    )
                else:
